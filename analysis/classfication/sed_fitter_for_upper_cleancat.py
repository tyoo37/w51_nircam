...
for ii, idx in enumerate(upper_idx):
    ...
    fitinfo = fit_a_source(data=tab_upper['flux'].quantity,
    ...
    dict_to_write[idx] = {
        ...
        'wavelength': np.asarray(tab_upper['wavelength']),
        'flux': np.asarray(tab_upper['flux']),
        'eflux': np.asarray(tab_upper['eflux']),
...
with open(f"/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/sed_fitting/reduced_chi2_staronly_upper_{os.getenv('SLURM_ARRAY_TASK_ID')}.pkl", "wb") as f:
