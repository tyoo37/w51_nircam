...
def plot_fit_models(fieldid, fits_dict, best_geo_r17, best_geo_gaussian ,apertures_arcsec, spicyid=0, label=None,
    ...
    geos = [best_geo_r17, best_geo_gaussian]
    if best_geo_r17 == best_geo_gaussian:
        geos = [best_geo_r17]
    
    # gather some information consistent across all geoms
    fitinfo = fits_dict[geos[0]]
    ...
    for geom in geos:       
        ...
        indexes = np.argsort(fits_dict[geom].chi2)[:10]
        for ii, index in enumerate(indexes):
            ...
            print(sed_flux_interp * distance_scale * av_scale_conv)
            ...
            _, = ax0.plot(sedcube.wav,
                    sed_flux_interp * distance_scale, color=colors[geom], ls='dotted',
                    alpha=0.9-ii*0.05)
    ...
    ax1.scatter(wavelengths_value, source.error, marker='*' ,color='k', alpha=0.9-ii*0.05, s=100)
    ...
    ax0.text(1e3, 1e6, 'chi2={:.2f}'.format(chi2), transform=ax0.transData)
    ax0.grid()
    ax1.grid()
    ...
    if label is not None:
        ax0.text(1e3, 1e5, label, transform=ax0.transData)
    ...
    for i,p in enumerate(params):
        ...
        for geo in geos:  
            ...
            index = np.argsort(fits_dict[geo].chi2)[:10]
            modelname_matched = fits_dict[geo].model_name[index]
            modelid_matched = fits_dict[geo].model_id[index]
...
for ii, idx in enumerate(lower_idx):
    ...
    if True:
        ...
        minchi2s = []
        for geo in all_gs:
            fitinfo = fits_dict[geo]

            minchi2s.append(np.nanmin(fitinfo.chi2))
        minchi2_global = np.nanmin(minchi2s)
        for geo in all_gs:
            fitinfo = fits_dict[geo]
            ...
            deltachi2 = fitinfo.chi2 - minchi2_global
            #prob = np.zeros(len(fitinfo.chi2))
            n_data = np.sum(valid[valid==1])
            print('geo:', geo, 'minchi2:', minchi2_global)
            print('n_data:', n_data)
            mask_prob = deltachi2 < 9 * n_data
            """
            pars = Table.read(f'/blue/adamginsburg/richardson.t/research/flux/pars/{geo}_augmented.fits')
            colnames = pars.colnames
            ...
            mask_colnames = ~np.isin(colnames, not_proper_cols)
            available_params = colnames[mask_colnames]
            n_model = len(available_params)-1

            """
            n_model = np.sum(np.isfinite(fitinfo.chi2))
            print('np.sum(mask_prob):', np.sum(mask_prob), 'n_model:', n_model)
            print(len(np.asarray(fitinfo.chi2)))
            likelihood_r17[geo] = np.sum(mask_prob)/n_model
            prob_gaussian = np.exp(-0.5 * deltachi2)
            print('np.nansum(prob_gaussian):', np.nansum(prob_gaussian), 'n_model:', n_model)
            likelihood_gaussian[geo] = np.nansum(prob_gaussian)/n_model
            print('geo:', geo, 'likelihood_r17:', likelihood_r17[geo], 'likelihood_gaussian:', likelihood_gaussian[geo])

        best_geo_r17 = max(likelihood_r17, key=likelihood_r17.get)
        best_geo_gaussian = max(likelihood_gaussian, key=likelihood_gaussian.get)
        print('best_geo_r17:', best_geo_r17, 'best_geo_gaussian:', best_geo_gaussian)
        best_geos_r17[idx] = best_geo_r17
        best_geos_gaussian[idx] = best_geo_gaussian
        

        if idx in matching_idx_w51e_arr:
            matching_idx = np.where(np.array(matching_idx_w51e_arr) == idx)[0][0]
            plot_fit_models('W51-E', fits_dict, best_geo_r17, best_geo_gaussian, fwhms_use, spicyid=idx,
            extinction=extinction, label=f'W51-E_{w51e_matched_idx[matching_idx]}', distance_range=[5.3,5.5]*u.kpc, extinction_range=[10, av_estimates[idx]],
            loc_imagedir=f'/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/sed_fitting/plots/{branchlabel}s/',
            robitaille_modeldir='/blue/adamginsburg/richardson.t/research/flux/r+24_models-1.2',)
        if idx in matching_idx_w51n_arr:
            matching_idx = np.where(np.array(matching_idx_w51n_arr) == idx)[0][0]
            plot_fit_models('W51-E', fits_dict, best_geo_r17, best_geo_gaussian, fwhms_use, spicyid=idx,
            extinction=extinction, label=f'W51-IRS2_{w51n_matched_idx[matching_idx]}', distance_range=[5.3,5.5]*u.kpc, extinction_range=[10, av_estimates[idx]],
            robitaille_modeldir='/blue/adamginsburg/richardson.t/research/flux/r+24_models-1.2',
            loc_imagedir=f'/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/sed_fitting/plots/{branchlabel}s/',)
        ...
        if idx%10==0:
            plot_fit_models('W51-E', fits_dict, best_geo_r17, best_geo_gaussian, fwhms_use, spicyid=idx,
            extinction=extinction, distance_range=[5.3,5.5]*u.kpc, extinction_range=[10, av_estimates[idx]],
            robitaille_modeldir='/blue/adamginsburg/richardson.t/research/flux/r+24_models-1.2',
            loc_imagedir=f'/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/sed_fitting/plots/{branchlabel}s/',)
            


with open(f"/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/sed_fitting/best_geos_r17_{branchlabel}_{os.getenv('SLURM_ARRAY_TASK_ID')}.pkl", "wb") as f:
    pickle.dump(best_geos_r17, f)

with open(f"/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/sed_fitting/best_geos_gaussian_{branchlabel}_{os.getenv('SLURM_ARRAY_TASK_ID')}.pkl", "wb") as f:
    pickle.dump(best_geos_gaussian, f)
