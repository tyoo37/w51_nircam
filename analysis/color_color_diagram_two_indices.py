...
imgsavedir = '/orange/adamginsburg/w51/TaehwaYoo/jwst_w51/plots/ccd_branches/ccds/'
filters = {
    'f140m': f140m_mag,
    'f162m': f162m_mag,
    'f182m': f182m_mag,
...
for i, (color1_pair, color2_pair) in enumerate(combinations(color_pairs, 2)):
    ...
    color1_idx1 = (filters[f1][0][idx1] - filters[f2][0][idx1]).value
    color2_idx1 = (filters[f3][0][idx1] - filters[f4][0][idx1]).value

    color1_idx2 = (filters[f1][0][idx2] - filters[f2][0][idx2]).value
    color2_idx2 = (filters[f3][0][idx2] - filters[f4][0][idx2]).value
    ...
    print(f"Finite values in color1 idx1: {np.isfinite(color1_idx1).sum()}, color2 idx1: {np.isfinite(color2_idx1).sum()}")
    print(f"Finite values in color1 idx2: {np.isfinite(color1_idx2).sum()}, color2 idx2: {np.isfinite(color2_idx2).sum()}")
    ...
    if np.isfinite(color1).sum()< 1000:
        size = 80
    else:
        size = 40
    ax.scatter(color1, color2, s=5, c='gray', alpha=0.5, edgecolor='none', label='All sources')
    ax.scatter(color1_idx1, color2_idx1, s=size*1.5, c='orange', marker='^', alpha=0.7, edgecolor='none', label='upper branch')
    ax.scatter(color1_idx2, color2_idx2, s=size, c='cyan', marker='s', alpha=0.7, edgecolor='none', label='lower branch')
    ...
    c1_mean = np.nanmean(color1_idx1)
    c1_std = np.nanstd(color1_idx1)
    c2_mean = np.nanmean(color2_idx1)
    c2_std = np.nanstd(color2_idx1)
