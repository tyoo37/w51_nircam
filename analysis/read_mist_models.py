...
class ISOCMD:
    ...
    def read_isocmd_file(self):
        ...
        hdr_list = None  # Initialize outside loop
        
        for i_age in range(num_ages):
            #grab info for each isochrone
            num_eeps = int(data[counter][-2])
            num_cols_header = int(data[counter][-1])
            
            # Determine actual number of columns from the first data row
            actual_num_cols = len(data[3+counter])
            
            # Read header list and truncate to actual columns
            hdr_list_full = data[counter+2][1:]
            hdr_list = hdr_list_full[:actual_num_cols]
            
            if actual_num_cols != num_cols_header:
                print(f"Age {i_age}: Header specifies {num_cols_header} columns, but data has {actual_num_cols}. Using {actual_num_cols}.")
            
            # Create formats tuple matching actual data
            formats = tuple([np.int32] + [np.float64 for i in range(actual_num_cols-1)])
            
            # Create structured array with correct dimensions
            isocmd = np.zeros((num_eeps), {'names': tuple(hdr_list), 'formats': formats})
            
            #read through EEPs for each isochrone
            for eep in range(num_eeps):
                isocmd_chunk = data[3+counter+eep]
            
                isocmd[eep] = tuple(isocmd_chunk[:actual_num_cols])
