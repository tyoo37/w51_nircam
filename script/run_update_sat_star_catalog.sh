...
mem=64gb
taskname=update_nircam_sat_stars_catalog
...
cd /home/t.yoo/w51/w51_nircam/catalog
#for filter in F140M F162M F182M F187N F210M F335M F360M F405N F410M F480M; do  
for filter in F162M; do
    sbatch --job-name=${taskname}-${filter} --output=${taskname}-${filter}_%j-%A.log  --account=astronomy-dept --qos=astronomy-dept-b --ntasks=2 --nodes=1 --mem=${mem} --time=96:00:00 --wrap "python /home/t.yoo/w51/w51_nircam/catalog/update_sat_stars_catalog.py --filter=${filter}"
done
