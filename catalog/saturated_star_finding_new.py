...
def finder_maker(max_size=100, min_size=0, min_sep_from_edge=5, min_flux=500,
    ...
    def saturated_finder(fitsdata, *args, raise_for_nosources=raise_for_nosources, rind_threshold=1000, **kwargs):
        """
        Wrap the star finder to reject bad stars
        """
        #saturated = np.logical_or((data==0), np.isnan(data))
        data = fitsdata['SCI'].data
        dqdata = fitsdata['DQ'].data
        data[np.isnan(fitsdata['VAR_POISSON'].data)] = 0

        saturated = (dqdata & dqflags.group['SATURATED']) > 0
...
def get_psf(header, path_prefix='.'):
    ...
    detector = header['DETECTOR'].lower()
    ww = wcs.WCS(header)
    try:
    ...
    if detector == 'nrcalong':
        ...
        detector = 'nrcb5'
    #psf_fn = f'{path_prefix}/{instrument.lower()}_{filtername}_samp{oversample}_nspsf{npsf}_npix{fov_pixels}_{module}_{detector}.fits'
    psf_fn = f'{path_prefix}/{instrument.lower()}_{detector}_{filtername.lower()}_fovp{fov_pixels}_samp{oversample}_npsf{npsf}.fits'
    if module == 'merged':
...
def iteratively_remove_saturated_stars(fitsdata, header,
    ...
    if header['INSTRUMENT'].lower() == 'nircam':
        psfgen = stpsf.NIRCam()
        fwhm, fwhm_pix = get_fwhm(header, instrument_replacement='NIRCam')
    elif header['INSTRUMENT'].lower() == 'miri':
        psfgen = stpsf.MIRI()
        fwhm, fwhm_pix = get_fwhm(header, instrument_replacement='MIRI')
    ...
    for (minsz, maxsz), minflx, grad, fitsz, apsz, diliter, rsz in zip(nsaturated, min_flux, require_gradient, fit_sizes, ap_rad, dilations, rindsize):
        ...
        if np.any((dq & dqflags.group['SATURATED']) > 0):
            sources = finder(resid, dq, 
        ...
        if diliter > 0:
            ...
            mask = ndimage.binary_dilation((dq & dqflags.group['SATURATED']) > 0, iterations=diliter)
        else:
        ...
        resid = phot.make_residual_image(resid, psf_shape=(fitsz, fitsz), include_localbkg=False)
...
def get_saturated_stars(fitsdata, minsz=100, maxsz=500, minflx=1000, grad=False, fitsz=351, apsz=15, rsz=6):
...
def remove_saturated_stars(filename, save_suffix='_unsatstar', **kwargs):
    ...
    #satstar_table, satstar_resid = iteratively_remove_saturated_stars(data, header, dq, **kwargs)
    satstar_table = get_saturated_stars(fh )
    if satstar_table is None or len(satstar_table) == 0:
        ...
        satstar_table.write(filename.replace(".fits", '_satstar_catalog_new.fits'), overwrite=True)
...
def main():
    ...
    for module in modules:
        for fn in glob.glob(f"/orange/adamginsburg/jwst/w51/{filt}/pipeline/*{module}*_cal.fits"):
            print()
            print(fn)
            remove_saturated_stars(fn, verbose=True)
