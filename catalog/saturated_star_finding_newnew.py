...
def iteratively_remove_saturated_stars(fitsdata,
    ...
    try:
        ...
        print("All parameter lists must be the same length", flush=True)
        print(len(fit_sizes), len(nsaturated), len(min_flux), len(ap_rad), len(dilations), len(require_gradient), flush=True)
        raise AssertionError("Parameter length mismatch")
    """
    header = fitsdata[0].header
    data = fitsdata['SCI'].data
    data[np.isnan(fitsdata['VAR_POISSON'].data)] = 0
    dq = fitsdata['DQ'].data
    data[(dq & dqflags.group['SATURATED'])>0] = -1
...
def get_saturated_stars(fitsdata, minsz=0, maxsz=50, minflx=1000, grad=False, fitsz=351, apsz=15, rsz=6, path_prefix='./', **kwargs):
    header = fitsdata[0].header
    data = fitsdata['SCI'].data
    data[np.isnan(fitsdata['VAR_POISSON'].data)] = 0
    dq = fitsdata['DQ'].data
...
def remove_saturated_stars(filename, save_suffix='_unsatstar', **kwargs):
    ...
    satstar_table, satstar_resid = iteratively_remove_saturated_stars(fh, **kwargs)
    #satstar_table, satstar_resid = get_saturated_stars(fh, **kwargs)
    if satstar_table is not None:
        ...
        print("No saturated stars found; skipping writing output files", flush=True)
...
def main():
    ...
    for module in ('nrca', 'nrcb'):
        for fn in glob.glob(f"/orange/adamginsburg/jwst/w51/{filt}/pipeline/*{module}*align*crf.fits"):
            print()
            print(fn)
            remove_saturated_stars(fn, verbose=True)
